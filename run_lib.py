...
#    batch = torch.from_numpy(inps).to(config.device).float()
    ...
    # weight = loadmat(r'/home/lqg/GY-T2-WKGM/datasets_fastMRI_img_IVI_12000/weight_qq.mat')[
    #   'global_weights']
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        kw_real, kw_imag = sample[0,:,:,0],sample[0,:,:,1] #(sample[0,:,:,0]+sample[0,:,:,2])/2, sample[0,:,:,1]
        kw_complex = kw_real+1j*kw_imag
        #weight = loadmat('/home/lqg/GY-T2-WKGM/datasetsweight/1weight220.mat')['weight']

        sample = np.log(1+np.abs(kw_complex))
        k_complex = kw_complex
        #k_complex = wgt2k(kw_complex, weight)
        samplek = np.log(1 + np.abs(k_complex))

        image0 = np.fft.ifft2(kw_complex)
        image = np.fft.ifft2(k_complex)
        write_kdata(sample,this_sample_dir,"samplekw.png")
        write_kdata(samplek, this_sample_dir, "samplek.png")

        write_data(image, this_sample_dir, "sample_ifft2.png")
        write_data(image0,this_sample_dir,"sample_ifftW2.png")
        #==========================================#               
